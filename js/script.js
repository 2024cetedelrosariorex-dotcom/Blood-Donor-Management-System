    ...
    drives: [
        {
            ...
            // scheduleDate: "2026-06-15", // [LAST CHECK - DISABLED] this date has already passed
            scheduleDate: "2026-12-15",
            barangayId: 7,
            status: "Scheduled"
        }
    ]
    ...
    document
        ...
        ?.addEventListener(
            ...
            event => {
                ...
                const city =
                    (document.getElementById("city") || { value: "General Santos City" }).value /* [REGISTER FORM - CHANGED] no City box any more */;


                const region =
                    (document.getElementById("region") || { value: "Region XII - SOCCSKSARGEN" }).value /* [REGISTER FORM - CHANGED] no Region box any more */;
                ...
                const payload = {
                    ...
                    registrarRoleId:
                        roleId,
                    // [REGISTER FORM - ADDED] the username typed by the staff member (the password is generated by the server)
                    username:
                        (document.getElementById("regUsername") || { value: "" }).value.trim()
                };
                ...
                .then(
                    result => {
                        ...
                        if (
                            ...
                            // [ORIGINAL - DISABLED] alert(`Donor registration successful! Generated Username: ${result.username || "Created by system"}`);
                            alert(buildRegistrationMessage(result, payload)); // [REGISTRATION - FIXED] also shows the temporary password when the email could not be sent
...
function validateUsernameField(
    ...
    if (field.id === "regUsername") {
        currentUserId = null; // [REGISTER FORM - ADDED] a new donor: no account is excused
    }
        ...
        field => {
            ...
            // -----------------------------
                ...
                [
                    ...
                    "regUsername", // [REGISTER FORM - ADDED]
                    "settingsNewUsername"
                ].includes(
                    field.id
                )
...
function addRegisteredDonorToRecords(payload, result) {
    ...
    // [DATABASE - ADDED] the real user id in the MySQL table USERS (used by api.php)
    const databaseUserId =
        result.donor && result.donor.userId
            ? Number(result.donor.userId)
            : (result.userId ? Number(result.userId) : null);

    // user id: use the one from the server if it does not clash with a sample account
    let newUserId =
        result.userId ?? result.userID ?? result.UserID ?? result.id ??
        (result.donor ? result.donor.userId : null) ?? null; // [REGISTRATION - FIXED] register_account.php sends it inside "donor"
    ...
    if (result.username) {
        ...
        db.users.push({
            ...
            roleName: "Volunteer Blood Donor",
            status: "Active",
            dbUserId: databaseUserId // [DATABASE - ADDED]
        });
    }
...
function askDeleteRecord(type, id) {
    ...
    if (type === "request") {
        ...
        // [DATABASE - ADDED] a donor who has a real login account in MySQL loses it too
        if (donor.dbUserId) {
            note =
                "The donor's real login account will also be DELETED from the database, together with the donation responses.";
        }
...
function confirmDeleteNow() {
    ...
    if (type === "request") {
        ...
        removeFromArray(db.alerts, item => item.requestId === id); // [EMERGENCY ALERTS - ADDED]
        ...
        removeFromArray(db.alerts, item => item.donorId === id); // [EMERGENCY ALERTS - ADDED]
...
function syncLoggedInDonor(result) {
    ...
    donor.userId = userId;
    donor.dbUserId = userId; // [DATABASE - ADDED] the real user id in the MySQL table USERS
...
// =========================================================
// [EMERGENCY ALERTS - ADDED] CHO / STAFF ALERT A DONOR  ->  DONOR SEES IT IN HIS DASHBOARD
// ---------------------------------------------------------
// Before: the "Alert Donor" button only showed a pop-up to the staff member and the
// donor's dashboard always showed the same fixed message, so no donor was really notified.
// Now: pressing "Alert Donor" saves an alert (Local Storage). When that donor logs in,
// the alert appears on his dashboard and in "My Donor Portal", and his answer
// (I Can Donate Now / I Can't Donate) comes back to the staff.
// Nothing above this line was removed or changed in logic.
// =========================================================
...
if (!RECORD_COLLECTIONS.includes("alerts")) {
    RECORD_COLLECTIONS.push("alerts");
}

// load the saved alerts too
loadRecords();


// --- 1. STAFF SIDE: send the alert ---
const originalNotifyDonorForEmergency = notifyDonorForEmergency;

notifyDonorForEmergency = function (donorId, requestId) {
    ...
    if (![1, 2, 3].includes(roleId)) {
        alert("You do not have permission to alert donors.");
        return;
    }
    ...
    if (!donor || !request) {
        alert("This donor or emergency request no longer exists.");
        return;
    }
    ...
    if (already) {
        alert("This donor was already alerted for this request.");
        return;
    }
    ...
    db.alerts.push({
        ...
        donorId: donorId,
        requestId: requestId,
        sentBy: db.currentUser.username,
    ...
    originalNotifyDonorForEmergency(donorId, requestId);

    // show "Alert Sent" in the match table right away
    markAlertedDonors();
};

// the match table shows what happened to each alert
function markAlertedDonors() {
    ...
    tbody.querySelectorAll("button").forEach(button => {
        ...
        const found =
            (button.getAttribute("onclick") || "")
                .match(/notifyDonorForEmergency\((\d+),\s*(\d+)\)/);
        ...
        const record =
            db.alerts.find(
                item =>
                    item.donorId === Number(found[1]) &&
                    item.requestId === Number(found[2])
            );
        ...
        if (record.status === "Responded") {
            ...
            badge.textContent = "Donor Responded";
        } else if (record.status === "Declined") {
            badge.className = "badge badge-warning";
            badge.textContent = "Donor Cannot Donate";
        } else {
            badge.className = "badge badge-warning";
...
const originalRenderMatchedDonors = renderMatchedDonors;

renderMatchedDonors = function () {

    originalRenderMatchedDonors.apply(this, arguments);

    markAlertedDonors();
};


// --- 2. DONOR SIDE: the alerts in "My Donor Portal" ---
function getLoggedInDonor() {
    ...
    return (
        db.donors.find(
            donor => donor.userId === db.currentUser.id
        ) || null
    );
}
...
function renderDonorAlerts() {
    ...
    const container =
        document.getElementById("donorNotificationsContainer");
    ...
    const mine =
        donor
            ? db.alerts
                .filter(item => item.donorId === donor.id)
                .filter(item => db.requests.some(request => request.id === item.requestId))
    ...
    if (mine.length === 0) {
        ...
        empty.textContent = "No pending emergency blood requests.";
        container.appendChild(empty);
        return;
    }

    mine.forEach(record => {
        // [DONATION APPOINTMENT - ADDED] once the donor answered YES, the "My Donation Status" cards below show everything
        if (
            record.status === "Responded" &&
            db.donations.some(
                item =>
                    item.donorId === record.donorId &&
        ...
        const request =
            db.requests.find(item => item.id === record.requestId);
        ...
        if (record.status === "Sent") {
            ...
            card.style.background = "var(--light-red)";
            card.style.borderLeft = "4px solid var(--garnet)";
            title.style.color = "var(--garnet)";
            title.textContent = "Emergency Call for Blood Donation!";

            text.textContent =
                `A patient at ${request.hospitalName} needs ` +
                `${request.quantity} bag(s) of ${getBloodTypeName(request.bloodTypeId)} blood ` +
                `(request date ${request.requestDate}). ` +
                `You were alerted by ${record.sentBy} on ${new Date(record.sentAt).toLocaleString()}. ` +
                `Please confirm if you can donate.`;
            ...
            yes.type = "button";
            yes.className = "btn-sm btn-alert";
            yes.style.marginTop = "6px";
            yes.style.marginRight = "8px";
            yes.textContent = "I Can Donate Now";
            yes.addEventListener("click", () => respondToAlert(record.id, true));

            const no = document.createElement("button");
            no.type = "button";
            no.className = "btn-sm btn-secondary";
            no.style.marginTop = "6px";
            no.textContent = "I Can't Donate";
            no.addEventListener("click", () => respondToAlert(record.id, false));
            ...
            title.textContent = "You said you can donate";
            text.textContent =
                `Emergency request #${request.id} at ${request.hospitalName}. ` +
                `The hospital staff will confirm your donation.`;
            card.append(title, text);
            ...
            title.textContent = "You said you cannot donate";
            text.textContent =
                `Emergency request #${request.id} at ${request.hospitalName}.`;
            card.append(title, text);
        }
...
function respondToAlert(alertId, willDonate) {

    const donor = getLoggedInDonor();

    const record =
        db.alerts.find(item => item.id === alertId);

    if (!donor || !record || record.donorId !== donor.id) {
        return;
    }
    ...
    if (willDonate) {
        ...
        alert("Thank you for letting us know. The health office was told that you cannot donate.");
    }

    refreshAllTables();
    renderDonorAlerts();
    updateDonorAlertBanner();
}

const originalRenderDonorPortal = renderDonorPortal;

renderDonorPortal = function () {

    originalRenderDonorPortal.apply(this, arguments);

    renderDonorAlerts();
};


// --- 3. DONOR DASHBOARD HOME: a red notice when there are new alerts ---
function updateDonorAlertBanner() {

    const dashboard =
        document.getElementById("entityDashDonor");

    if (!dashboard) {
        return;
    }

    let banner = document.getElementById("donorAlertBanner");

    if (!banner) {

        banner = document.createElement("div");
        banner.id = "donorAlertBanner";
        banner.className = "donor-alert-banner";
        ...
        donor
            ? db.alerts.filter(
                item =>
                    item.donorId === donor.id &&
                    item.status === "Sent" &&
                    db.requests.some(request => request.id === item.requestId)
            ).length
            : 0;
    ...
    message.textContent =
        count === 1
            ? "You have 1 new emergency blood alert. Please check and answer it."
            : `You have ${count} new emergency blood alerts. Please check and answer them.`;

    const open = document.createElement("button");
    open.type = "button";
    open.className = "btn-sm btn-alert";
    open.textContent = "View Alerts";
    open.addEventListener("click", () => switchModuleView("viewMyProfile"));

    banner.append(message, open);
}

const originalRenderHomeDashboardForDonorAlerts = renderHomeDashboardForUser;

renderHomeDashboardForUser = function () {

    originalRenderHomeDashboardForDonorAlerts.apply(this, arguments);
...
// =========================================================
// [REGISTRATION - ADDED] MESSAGE AFTER REGISTERING A DONOR
// register_account.php creates the username (firstname.lastname) and a random temporary
// password (Donor + 4 digits + !, for example Donor4821!) and tries to email both with PHP mail().
// XAMPP usually cannot send email, so when it fails the staff member needs to see the password.
// =========================================================
function buildRegistrationMessage(result, payload) {

    const username = result.username || "Created by system";
    ...
    if (result.emailSent) {
        ...
        message +=
            "\nThe username and temporary password were emailed to " + email + ".";
        ...
        message +=
            "\nThe email could NOT be sent, so please write down the username and " +
            "password above and give them to the donor.\n" +
            "The donor can change the password in Settings after logging in.";
    }
...
// =========================================================
// [DONATION APPOINTMENT - ADDED] WHAT HAPPENS AFTER THE DONOR SAYS "I CAN DONATE"
// ---------------------------------------------------------
// Before: "Confirm Donation" only changed a status. The donor was never told where
// or when to go.
// Now:
//   1. Donor answers "I Can Donate Now"            -> status: Awaiting Confirmation
//   2. Hospital staff click "Confirm Donation" and choose the DATE, TIME and
//      instructions                                 -> status: Confirmed
//      The donor sees WHERE (hospital) and WHEN in his dashboard.
//   3. After the donor gives blood, staff click
//      "Mark as Donated"                            -> status: Completed
//      The donor gets a thank-you and the date he can donate again (90 days).
// Nothing above this line was removed or changed in logic.
// =========================================================

FORM_PERMISSIONS.formConfirmDonation = [1, 2];
...
function formatAppointmentDate(text) {
...
function formatAppointmentTime(text) {
...
function getDonationPlace(donation) {

    const request =
        db.requests.find(item => item.id === donation.requestId);

    if (!request) {
        return "the hospital";
    }

    const hospital = request.hospitalName || "the hospital";

    const barangay =
        request.barangayId
            ? getBarangayName(request.barangayId)
            : "";

    return barangay
        ? `${hospital}, Brgy. ${barangay}`
        : hospital;
}


// --- 1. STAFF: "Confirm Donation" now asks for the date, time and instructions ---
const originalConfirmDonation = confirmDonation;   // old version (kept, no longer used)

confirmDonation = function (donationId) {
    ...
    if (![1, 2].includes(roleId)) {
        alert("You do not have permission to confirm donations.");
        return;
    }

    const donation =
        db.donations.find(item => item.id === donationId);

    if (!donation || donation.status !== "Awaiting Confirmation") {
        return;
    }
    ...
    clearFormValidation(
        document.getElementById("formConfirmDonation")
    );

    setFieldValue("confirmDonationId", donation.id);
    setFieldValue("confirmDonationDate", "");
    setFieldValue("confirmDonationTime", "");
    setFieldValue("confirmDonationNote", "");

    const dateField = document.getElementById("confirmDonationDate");

    if (dateField) {
        dateField.min = todayLocalText();
        dateField.max = yearsAheadText(1); // [DATE LIMIT - ADDED] blocks impossible years such as 62026
    }

    const summary = document.getElementById("confirmDonationSummary");

    if (summary) {
        summary.textContent =
            `Donor: ${getDonorName(donor)} (${getBloodTypeName(donation.bloodTypeId)}) - ` +
            `emergency request #${donation.requestId} at ${getDonationPlace(donation)}.`;
    }

    openModal("confirmDonationModal");
};

function submitConfirmDonation(event) {
    ...
    const donation =
        db.donations.find(
            item => item.id === parseInt(getFieldValue("confirmDonationId"))
        );

    if (!donation || donation.status !== "Awaiting Confirmation") {
        alert("This donation can no longer be confirmed.");
        closeModal("confirmDonationModal");
        refreshAfterRecordChange();
        return;
    }

    const date = getFieldValue("confirmDonationDate");
    const time = getFieldValue("confirmDonationTime");

    // a time that already passed today is not allowed
    if (date === todayLocalText()) {
        ...
        if (time < nowText) {
            showValidationError(
                document.getElementById("confirmDonationTime"),
    ...
    donation.appointmentDate = date;
    donation.appointmentTime = time;
    donation.appointmentNote = getFieldValue("confirmDonationNote");
    donation.confirmedBy = db.currentUser.username;
    donation.confirmedAt = new Date().toISOString();

    closeModal("confirmDonationModal");

    alert("Appointment confirmed! The donor will see the place, date and time in his dashboard.");

    refreshAfterRecordChange();
}


// --- 2. STAFF: "Mark as Donated" after the donor gave blood ---
function completeDonation(donationId) {
    ...
    if (![1, 2].includes(roleId)) {
        alert("You do not have permission to complete donations.");
        return;
    }
    ...
    if (!confirm("Mark this donation as completed? Do this only after the donor gave blood.")) {
        return;
    }
    ...
    // the donor rests for 90 days before the next donation
    const donor = db.donors.find(item => item.id === donation.donorId);
    ...
    // the request is fulfilled when enough bags were received
    const request =
        db.requests.find(item => item.id === donation.requestId);

    let message = "Donation completed. Thank you!";
    ...
    if (request) {
        ...
        if (received >= Number(request.quantity)) {
            request.status = "Fulfilled";
            message += `\n\n${received} of ${request.quantity} bag(s) received. The request is now Fulfilled.`;
        } else {
            message += `\n\n${received} of ${request.quantity} bag(s) received. The request stays Pending until all bags are received.`;
        }
    }
...
// the "Pending Donation Confirmations" table shows the appointment and the right button
const originalRenderPendingDonations = renderPendingDonations;

renderPendingDonations = function () {

    originalRenderPendingDonations.apply(this, arguments);
    ...
    tbody.querySelectorAll("tr").forEach(row => {
        ...
        const id = parseInt(cells[0].textContent.replace("#", "").trim());
        ...
        if (donation.status === "Confirmed") {
            ...
            const badge = document.createElement("span");
            badge.className = "badge badge-warning";
            badge.textContent = "Appointment Set";
            statusCell.appendChild(badge);

            if (donation.appointmentDate) {
                const when = document.createElement("div");
                when.className = "settings-help";
                when.textContent =
                    formatAppointmentDate(donation.appointmentDate) +
                    " at " +
                    formatAppointmentTime(donation.appointmentTime);
                statusCell.appendChild(when);
            }
            ...
            done.className = "btn-sm btn-success";
            done.textContent = "Mark as Donated";
            done.addEventListener("click", () => completeDonation(donation.id));
            ...
            finished.className = "settings-help";
            finished.textContent = "Donation done";
            actionCell.appendChild(finished);
        }
    });
};


// --- 3. DONOR: "My Donation Status" (where and when) in My Donor Portal ---
function appendDonationStatusCards() {
    ...
    heading.textContent = "My Donation Status";
    heading.style.margin = "16px 0 8px";
    container.appendChild(heading);

    const addLine = (card, label, value) => {
    ...
    mine.forEach(donation => {
        ...
        if (donation.status === "Awaiting Confirmation") {

            card.style.background = "#f8fafc";
            card.style.borderLeft = "4px solid #94a3b8";
            title.textContent = "Waiting for the hospital to confirm your donation";

            addLine(card, "Request:", `#${donation.requestId} at ${getDonationPlace(donation)}`);
            addLine(card, "Next:", "The hospital will set the date and time. You will see it here.");
            ...
            card.style.borderLeft = "4px solid #16a34a";
            title.style.color = "#166534";
            title.textContent = "Your donation is confirmed!";

            addLine(card, "Where:", getDonationPlace(donation));
            ...
            if (donation.appointmentDate) {
                ...
                addLine(card, "When:", "The hospital will contact you.");
            }

            if (donation.appointmentNote) {
                addLine(card, "Instructions:", donation.appointmentNote);
            }

            addLine(card, "Please:", "Bring a valid ID and drink water before you go.");
            ...
            title.textContent = "Thank you for donating blood!";

            addLine(
                card,
                "Donated on:",
                formatAppointmentDate(
                    (donation.completedAt || "").split("T")[0] || donation.date
                )
            );
            ...
            if (donor.nextEligibleDate) {
                addLine(
                    ...
                    formatAppointmentDate(donor.nextEligibleDate)
                );
            }
        }
...
const originalRenderDonorAlertsForDonations = renderDonorAlerts;

renderDonorAlerts = function () {

    originalRenderDonorAlertsForDonations.apply(this, arguments);

    appendDonationStatusCards();
};

// --- 4. DONOR DASHBOARD HOME: green notice with the confirmed appointment ---
const originalUpdateDonorAlertBannerForDonations = updateDonorAlertBanner;

updateDonorAlertBanner = function () {

    originalUpdateDonorAlertBannerForDonations.apply(this, arguments);

    const banner = document.getElementById("donorAlertBanner");
    const donor = getLoggedInDonor();
    ...
    banner.classList.remove("donor-alert-banner--ok");

    // new emergency alerts (red) come first
    if (!donor || banner.style.display !== "none") {
        return;
    }

    const confirmed =
        db.donations
            .filter(item => item.donorId === donor.id && item.status === "Confirmed")
    ...
    banner.classList.add("donor-alert-banner--ok");
    banner.style.display = "flex";
    banner.innerHTML = "";
    ...
    message.textContent =
        confirmed.appointmentDate
            ? `Your donation is confirmed: ${formatAppointmentDate(confirmed.appointmentDate)} at ${formatAppointmentTime(confirmed.appointmentTime)}, ${getDonationPlace(confirmed)}.`
            : `Your donation is confirmed at ${getDonationPlace(confirmed)}. The hospital will contact you.`;
...
// connect the new form
document.addEventListener(
    "DOMContentLoaded",
    () => {

        const form = document.getElementById("formConfirmDonation");

        if (form) {
            form.addEventListener("submit", submitConfirmDonation);
        }
    }
);
...
// =========================================================
// [DONATION APPOINTMENT - ADDED] A donor who finished the 90-day rest becomes Available again
// (before, "Unavailable" stayed forever unless staff clicked Toggle Status)
// =========================================================
function restoreEligibleDonors() {
    ...
    db.donors.forEach(donor => {
        ...
        if (
            ...
            delete donor.nextEligibleDate;
        }
    });
}

const originalRefreshAllTablesForEligibility = refreshAllTables;

refreshAllTables = function () {

    restoreEligibleDonors();
    repairBadAppointments(); // [DATE LIMIT - ADDED]

    originalRefreshAllTablesForEligibility.apply(this, arguments);
};


// =========================================================
// [DONOR HOME - ADDED] THE DONOR SEES HIS OWN NUMBERS
// The four cards on the home page are system-wide statistics (all registered donors,
// all pending requests ...). Those are for the staff. A donor now sees his own:
//   My Completed Donations | New Emergency Alerts | Available to Donate | Upcoming Blood Drives
// The staff cards are unchanged.
// =========================================================
const ORIGINAL_HOME_LABELS = {
    statTotalDonors: "Registered Donors",
    statActiveRequests: "Pending Emergency Requests",
    statVerifiedDonors: "Verified Active Donors",
    statUpcomingDrives: "Upcoming Blood Drives"
};

function applyDonorHomeStats() {
    ...
    const donorLabels = {
        statTotalDonors: "My Completed Donations",
        statActiveRequests: "New Emergency Alerts",
        statVerifiedDonors: "Available to Donate",
        statUpcomingDrives: "Upcoming Blood Drives"
    };

    // labels (donor = own numbers, everyone else = original text)
    Object.keys(ORIGINAL_HOME_LABELS).forEach(id => {

        const value = document.getElementById(id);

        if (!value || !value.nextElementSibling) {
            return;
        }
    ...
    setValue("statTotalDonors", completed);
    setValue("statActiveRequests", newAlerts);
    setValue(
        "statVerifiedDonors",
        !donor
            ? "-"
    ...
    setValue("statUpcomingDrives", countUpcomingDrives()); // [LAST CHECK - CHANGED] only drives that have not happened yet
}

const originalRenderHomeDashboardForDonorStats = renderHomeDashboardForUser;

renderHomeDashboardForUser = function () {

    originalRenderHomeDashboardForDonorStats.apply(this, arguments);

    applyDonorHomeStats();
};

// keep the numbers right after the donor answers an alert
const originalUpdateDonorAlertBannerForStats = updateDonorAlertBanner;

updateDonorAlertBanner = function () {

    originalUpdateDonorAlertBannerForStats.apply(this, arguments);
...
// =========================================================
// [DATE LIMIT - ADDED] IMPOSSIBLE YEARS (for example 62026) WERE ACCEPTED
// A date box lets you type up to 6 digits for the year, and the fields only had a
// minimum date. Now every "future" date has a maximum too, and old bad appointments
// are repaired automatically.
// =========================================================
function yearsAheadText(years) {
...
// a saved appointment with a date like 62026-02-09 goes back to "Awaiting Confirmation"
function repairBadAppointments() {

    db.donations.forEach(donation => {

        if (
            donation.status === "Confirmed" &&
            !/^(20\d\d)-\d{2}-\d{2}$/.test(String(donation.appointmentDate || ""))
        ) {

            donation.status = "Awaiting Confirmation";

            delete donation.appointmentDate;
            delete donation.appointmentTime;
            delete donation.appointmentNote;
            delete donation.confirmedBy;
            delete donation.confirmedAt;
        }
    });
}
...
document.addEventListener(
    ...
    () => {
        ...
        // future dates: at most 2 years ahead (an emergency request date: 1 year)
        [
            ["inlineReqDate", 1],
            ["reqDate", 1],
            ["editRequestDate", 1],
            ...
            if (field) {
                field.max = yearsAheadText(years);
            }
        });
    }
);

repairBadAppointments();


// =========================================================
// [REGISTER FORM - ADDED] Username box: live check (format + already used)
// =========================================================
document.addEventListener(
    "DOMContentLoaded",
    () => {
...
// =========================================================
// [REGISTER FORM - SIMPLIFIED] NO REGION / CITY BOXES
// The system is for General Santos City, so Region and City were taken out of the form.
// The barangay list is filled directly, and the server still receives
// city = General Santos City and region = Region XII - SOCCSKSARGEN.
// =========================================================
function fillRegisterBarangays() {

    const barangay = document.getElementById("barangay");

    // if the old City box still exists, the old code fills the barangays
    if (!barangay || document.getElementById("city")) {
        return;
    }

    barangay.innerHTML = '<option value="">Select Barangay</option>';

    db.barangays.forEach(item => {
...
document.addEventListener("DOMContentLoaded", fillRegisterBarangays);


// =========================================================
// [LAST CHECK - ADDED] SMALL FIXES FOUND IN THE FINAL CHECK
// =========================================================

// --- 1. "Upcoming Blood Drives" counted every drive, even ones in the past ---
function countUpcomingDrives() {

    const today = todayLocalText();

    return db.drives.filter(
        drive =>
            String(drive.scheduleDate || "") >= today &&
            drive.status !== "Cancelled" &&
            drive.status !== "Completed"
    ).length;
}

const originalRenderHomeDashboardForDriveCount = renderHomeDashboardForUser;

renderHomeDashboardForUser = function () {

    originalRenderHomeDashboardForDriveCount.apply(this, arguments);

    const isDonor =
        db.currentUser &&
        Number(db.currentUser.roleId) === 4;

    const card = document.getElementById("statUpcomingDrives");

    if (card && !isDonor) {
        card.textContent = countUpcomingDrives();
    }
};

// --- 2. clearer error messages (before: "Please check this field and try again.") ---
const originalGetValidationMessage = getValidationMessage;
...
getValidationMessage = function (field) {
    ...
    if (field && field.validity) {
        ...
        if (v.stepMismatch) {
            return "Please enter a whole number, for example 1 or 2.";
        }
        ...
        const otherProblem =
            v.valueMissing || v.tooShort || v.tooLong ||
            v.rangeUnderflow || v.rangeOverflow;

        if (v.patternMismatch && !otherProblem) {

            const pattern = field.pattern || "";

            if (/A-Za-z/.test(pattern) && !/0-9/.test(pattern)) {
                return "Please use letters only. Numbers and symbols are not allowed.";
            }

            if (/A-Za-z/.test(pattern) && /0-9/.test(pattern)) {
                return "Please use only letters, numbers, dots, underscores or hyphens.";
            }

            if (/\\\+63/.test(pattern)) {
                return "Please enter +63 followed by 10 digits.";
            }

            return "Please use the correct format for this field.";
        }
    }

    return originalGetValidationMessage.apply(this, arguments);
};

// --- 3. text boxes must not contain < or > (stops HTML from being typed into the tables) ---
const originalValidateFormForSymbols = validateForm;

validateForm = function (form) {

    const result = originalValidateFormForSymbols.apply(this, arguments);
    ...
    if (form && form.querySelectorAll) {
        ...
        form
            ...
            .forEach(field => {
                ...
                showValidationError(
                    field,
                    "Please do not use the < or > symbols."
                );
    ...
    return result && symbolsOk;
};


// =========================================================
// [DATABASE - ADDED] EVERY FORM IS SAVED IN THE MYSQL DATABASE (through api.php)
// ---------------------------------------------------------
// Before: records lived only in this browser (Local Storage).
// Now: every time a form adds, edits or deletes a record, the change is also sent to
// api.php, which stores it in MySQL (table app_records; donor edits are copied to the real
// tables USERS and Volunteer_Blood_donor). When someone logs in, the saved records are loaded
// from MySQL, so every computer and every browser sees the same data.
// If the database cannot be reached, the system keeps working with Local Storage
// and shows "Database: offline".
// Nothing above this line was removed or changed in logic.
// =========================================================

const SERVER_COLLECTIONS = ["users", "donors", "requests", "drives", "donations", "alerts"];
const SERVER_SNAPSHOT_KEY = "bloodDonorServerSnapshot_v1";

const serverSync = {
    status: "idle",       // idle | connecting | connected | saving | offline
    inFlight: false,
    again: false,
    timer: null,
    pollTimer: null,
    polling: false,
    snapshot: null        // what the database has (collection -> id -> text)
};
...
function cleanDocForServer(collection, doc) {

    const copy = JSON.parse(JSON.stringify(doc));

    if (collection === "users") {
        delete copy.password; // passwords are never sent
    }

    return copy;
}

// the state of all records as text: { collection: { id: "text" } }
function currentServerState() {

    const state = {};

    SERVER_COLLECTIONS.forEach(name => {
        ...
        (db[name] || []).forEach(doc => {
            ...
            if (doc && doc.id !== undefined && doc.id !== null) {
                state[name][String(doc.id)] =
                    canonicalJson(cleanDocForServer(name, doc));
            }
        });
    });
...
function saveServerSnapshot(state) {

    serverSync.snapshot = state;

    try {
        localStorage.setItem(SERVER_SNAPSHOT_KEY, JSON.stringify(state));
    } catch (error) {
        // ignored: only used to find changes made while offline
    }
}

function currentRoleId() {

    return db.currentUser ? Number(db.currentUser.roleId) : 0;
}

// --- the small "Database: ..." label at the bottom left ---
function setDatabaseStatus(status) {

    serverSync.status = status;
    ...
    const labels = {
        ...
        saving: "Database: saving...",
        offline: "Database: offline (saved in this browser only)"
    };

    if (!db.currentUser || !labels[status]) {
...
// --- find what changed since the last time the database and this page agreed ---
function computeServerChanges(state, snapshot, roleId) {

    const isStaff = [1, 2, 3].includes(roleId);
    const changes = {};
    let count = 0;

    SERVER_COLLECTIONS.forEach(name => {

        if (!isStaff && !["donors", "donations", "alerts"].includes(name)) {
            return;
        }
        ...
        Object.keys(now).forEach(id => {
            ...
            if (before[id] !== now[id]) {
                ...
                const doc =
                    (db[name] || []).find(item => String(item.id) === id);

                if (doc) {
                    upsert.push(attachDatabaseHints(name, cleanDocForServer(name, doc)));
                }
            }
        });
        ...
        if (upsert.length || remove.length) {
            changes[name] = { upsert: upsert, delete: remove };
            count += upsert.length + remove.length;
        }
    });
...
// donors that have a real login also send their barangay / blood type NAMES,
// so api.php can copy the changes to the real table Volunteer_Blood_donor
function attachDatabaseHints(collection, doc) {

    if (collection === "donors" && doc.dbUserId) {

        const barangay = db.barangays.find(item => item.id === doc.barangayId);
        const bloodType = db.bloodTypes.find(item => item.id === doc.bloodTypeId);

        if (barangay) {
            doc._barangayName = barangay.name;
        }

        if (bloodType) {
            doc._bloodTypeName = bloodType.name;
        }
    }
...
async function syncNow(initialize) {

    if (!db.currentUser) {
        return false;
    }

    if (serverSync.inFlight) {
        serverSync.again = true;
        return false;
    }
    ...
    let snapshot = serverSync.snapshot || loadServerSnapshot();

    if (!snapshot) {
        if (!initialize) {
            return false; // this browser has never talked to the database yet
        }
        snapshot = {};
    }

    const result = computeServerChanges(state, snapshot, roleId);

    if (result.count === 0 && !initialize) {
        serverSync.snapshot = snapshot;
        return true;
    }

    serverSync.inFlight = true;
    setDatabaseStatus("saving");

    try {

        const response = await fetch("api.php?action=sync", {
            method: "POST",
        ...
        serverSync.inFlight = false;

        if (serverSync.again) {
            serverSync.again = false;
            scheduleServerSync();
        }
    }
}
...
function scheduleServerSync() {
    ...
    serverSync.timer = setTimeout(() => {
        syncNow(false);
    }, 400);
}

// every save of the records also saves them in the database
const originalSaveRecordsForServer = saveRecords;

saveRecords = function () {

    const result = originalSaveRecordsForServer.apply(this, arguments);
...
function applyServerCollections(collections) {

    SERVER_COLLECTIONS.forEach(name => {

        if (Array.isArray(collections[name])) {
            db[name].splice(0, db[name].length, ...collections[name]);
        }
    });
}

function serverStateFromCollections(collections) {
    ...
    SERVER_COLLECTIONS.forEach(name => {
        ...
        (collections[name] || []).forEach(doc => {
            if (doc && doc.id !== undefined && doc.id !== null) {
                state[name][String(doc.id)] = canonicalJson(doc);
            }
        });
    });
...
async function fetchServerRecords() {
    ...
    if (!data || !data.success) {
        throw new Error(data && data.message ? data.message : "load failed");
    }
...
async function pullFromServer() {
    ...
    if (!data.initialized) {
        ...
        // the database is empty: the first staff member who logs in saves the starting data
        if ([1, 2, 3].includes(roleId)) {
            saveServerSnapshot({});
    ...
    applyServerCollections(data.collections || {});

    // remember what the database has (canonical text of what was received)
    const stateNow = currentServerState();
    saveServerSnapshot(stateNow);

    originalSaveRecordsForServer(); // Local Storage copy (does not start another sync)

    return true;
}

// --- LOGIN: send left-over offline changes, then load the database ---
let pendingLoginResult = null;

const originalSyncLoggedInDonor = syncLoggedInDonor;

// the donor's own profile is added AFTER the database records are loaded
syncLoggedInDonor = function (result) {
    pendingLoginResult = result;
};

const originalLoginUserSuccess = loginUserSuccess;
...
loginUserSuccess = function () {
    ...
    startDatabaseSession().finally(() => {
        originalLoginUserSuccess.apply(self, args);
    });
};
...
async function startDatabaseSession() {
    ...
    try {
        ...
        await syncNow(false);     // changes made while the database was offline
        await pullFromServer();   // records saved by everybody

        setDatabaseStatus("connected");

    } catch (error) {

        console.warn("Database not reachable:", error);
        setDatabaseStatus("offline");
    }

    try {
        originalSyncLoggedInDonor(pendingLoginResult);
    } catch (error) {
        console.warn(error);
    }
...
// --- keep the page up to date: look at the database every few seconds ---
function startDatabasePolling() {

    clearInterval(serverSync.pollTimer);

    serverSync.pollTimer =
        setInterval(pollDatabase, window.SERVER_POLL_MS || 15000);
}
...
async function pollDatabase() {
    ...
    // do not disturb a window that is open
    const openModal =
    ...
    // my own changes first
    const snapshot = serverSync.snapshot || loadServerSnapshot();

    if (!snapshot) {
        return;
    }

    if (computeServerChanges(currentServerState(), snapshot, currentRoleId()).count > 0) {
        scheduleServerSync();
        return;
    }

    serverSync.polling = true;
    ...
    try {
        ...
        const incoming = serverStateFromCollections(data.collections || {});

        const changed =
            SERVER_COLLECTIONS.some(name => {

                if (!Array.isArray((data.collections || {})[name])) {
                    return false;
                }

                return canonicalJson(incoming[name]) !== canonicalJson(snapshot[name] || {});
            });
        ...
        applyServerCollections(data.collections || {});
        saveServerSnapshot(currentServerState());

        refreshAllTables();

        if (typeof renderHomeDashboardForUser === "function") {
            renderHomeDashboardForUser();
        }
        ...
        serverSync.polling = false;
    }
}

// --- LOGOUT: send the last changes, then end the login session ---
const originalLogoutCurrentUserForServer = logoutCurrentUser;
...
logoutCurrentUser = function () {
    ...
    const lastSave = syncNow(false); // starts now, before the user is cleared

    originalLogoutCurrentUserForServer.apply(this, arguments);

    lastSave.finally(() => {
        fetch("api.php?action=logout", { credentials: "same-origin" }).catch(() => {});
    });

    serverSync.snapshot = null;

    setDatabaseStatus("idle");
};

// --- RESET TO SAMPLE DATA: erase the database records too ---
resetSavedRecords = function () {

    const sure = confirm(
        "This will erase ALL saved records (in this browser and in the database) and restore the original sample data.\n\nContinue?"
    );
