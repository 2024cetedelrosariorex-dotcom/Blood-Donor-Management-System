...
try {
    ...
    if ($action === 'sync') {
        ...
        // ---- STAFF USER ACCOUNTS (Manage User Accounts - CHO Admin only, roles 2/3) ----
        if ($roleId === 1) {
            foreach (($changes['users']['upsert'] ?? []) as $doc) {
                $targetId = (int)($doc['id'] ?? 0);
                $newUsername = trim((string)($doc['username'] ?? ''));
                if (!$targetId || $targetId === $userId || !preg_match('/^[A-Za-z0-9._-]{3,30}$/', $newUsername)) {
                    continue;
                }
                $target = $pdo->prepare("SELECT ROLES_RoleID FROM USERS WHERE UserID = ? LIMIT 1");
                $target->execute([$targetId]);
                $targetRole = $target->fetchColumn();
                if (!in_array((int)$targetRole, [2, 3], true)) {
                    continue; // this form only manages Hospital Staff / Barangay Health Worker accounts
                }
                $dupe = $pdo->prepare("SELECT UserID FROM USERS WHERE LOWER(Username) = LOWER(?) AND UserID != ? LIMIT 1");
                $dupe->execute([$newUsername, $targetId]);
                if ($dupe->fetch()) {
                    continue; // username already taken - skip rather than fail the whole save
                }
                $pdo->prepare("UPDATE USERS SET Username = ? WHERE UserID = ?")->execute([$newUsername, $targetId]);
                $saved++;
            }
            foreach (($changes['users']['delete'] ?? []) as $id) {
                $id = (int)$id;
                if (!$id || $id === $userId) continue;
                $target = $pdo->prepare("SELECT ROLES_RoleID FROM USERS WHERE UserID = ? LIMIT 1");
                $target->execute([$id]);
                $targetRole = (int)$target->fetchColumn();
                if ($targetRole === 2) {
                    $pdo->prepare("DELETE FROM hospital_stf WHERE USERS_UserID = ?")->execute([$id]);
                } elseif ($targetRole === 3) {
                    $pdo->prepare("DELETE FROM barangay_health_worker WHERE USERS_UserID = ?")->execute([$id]);
                } else {
                    continue; // only Hospital Staff / Barangay Health Worker accounts are removable here
                }
